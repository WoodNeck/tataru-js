...
module.exports = {
	...
	execute: async ({ bot, author, guild, channel, content }) => {
		if (author.id === global.env.BOT_DEV_USER_ID) {
			const stamps = content === STATS.DAILY
				? await StampLog.find({ time: new Date().toDateString() })
					.sort({ callCount: -1 })
					.limit(STATS.MAX_ENTRY)
					.exec()
				: await StampLog.find({ time: undefined })
					.sort({ callCount: -1 })
					.limit(STATS.MAX_ENTRY)
					.exec();
			...
			const resultEmbed = new MessageEmbed()
				.setTitle(content === STATS.DAILY ? STATS.RESULT_DEV_DAILY_TITLE : STATS.RESULT_DEV_TITLE)
